...
## 3. Polling events via CLI instead of a Monitor

Running `codex-team -b $TOK status` in a loop from Bash burns context and misses events. Use a persistent Monitor (`/codex-team:events`) so events land in your task panel asynchronously.
...
## 11. Reading events as the source of truth for turn content

Events are summaries. Turn CONTENT (assistant messages, shell output, diffs) is reconstructible only from `item.completed` notifications during the turn, or from codex's own rollout files on disk. Don't expect `turn.completed.payload` to contain anything beyond metadata.

## 12. Assuming `session new` plus nothing = a session you can reattach later
...
## 13. One bearer token across many unrelated projects
...
## 14. Running monitor alarm for heartbeats you could replace with events
...
## 15. Spinning up a hierarchical manager for a 2-task problem
